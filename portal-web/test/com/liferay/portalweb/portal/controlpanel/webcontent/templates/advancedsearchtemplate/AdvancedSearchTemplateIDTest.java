 ...
 */
public class AdvancedSearchTemplateIDTest extends BaseTestCase {
	public void testAdvancedSearchTemplateID() throws Exception {
		int label = 1;
		...
		while (label >= 1) {
			...
			case 2:
				...
				for (int second = 0;; second++) {
					...
					try {
						if (selenium.isVisible("_15_searchTemplateId")) {
							break;
						}
					}
					catch (Exception e) {
				...
				selenium.type("_15_searchTemplateId",
					RuntimeVariables.replace("templateid1"));
				selenium.saveScreenShotAndSource();
				selenium.clickAt("//div[2]/span[2]/span/input",
					RuntimeVariables.replace("Search"));
				selenium.waitForPageToLoad("30000");
