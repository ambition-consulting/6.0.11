...
public class ActivateStagingCommunityTest extends BaseTestCase {
	public void testActivateStagingCommunity() throws Exception {
		...
		for (int second = 0;; second++) {
			...
			try {
				if (selenium.isVisible("_29_stagingEnabled")) {
					break;
				}
			}
			catch (Exception e) {
		...
		selenium.saveScreenShotAndSource();
		assertFalse(selenium.isChecked("_29_stagingEnabled"));
		selenium.saveScreenShotAndSource();
		selenium.clickAt("_29_stagingEnabled",
		...
		assertTrue(selenium.isChecked("_29_stagingEnabled"));
		selenium.saveScreenShotAndSource();
	}
}